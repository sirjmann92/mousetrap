...
def test_vault_connection_config():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # Save the session
            save_response = requests.post(f"{base_url}/api/session/save", 
                                        json=session_data,
    ...
    try:
        ...
        test_data = {
            "label": "Gluetun",
            "browser_mam_id": "test_cookie_value"
        }
        
        validation_response = requests.post(f"{base_url}/api/session/validate_cookies",
                                          json=test_data,
                                          headers={'Content-Type': 'application/json'})
