...
from backend.config import load_session, save_session, list_sessions
    ...
    try:
        ...
        for label in session_labels:
            try:
                ...
                logging.warning(f"[get_sessions_by_uid] Error loading session {label}: {e}")
                continue
                
    except Exception as e:
        logging.error(f"[get_sessions_by_uid] Error: {e}")
    ...
    try:
        ...
        for session_data in uid_sessions:
            try:
                ...
                logging.warning(f"[sync_browser_mam_id_across_uid_sessions] {error_msg}")
                
        if result['updated_sessions']:
            result['success'] = True
            result['message'] = f"Browser MAM ID synced across {len(result['updated_sessions'])} sessions for UID {target_uid}"
        else:
            result['message'] = "No sessions were updated"
            
    except Exception as e:
        result['message'] = f'Sync error: {str(e)}'
        logging.error(f"[sync_browser_mam_id_across_uid_sessions] Error: {e}")
...
def check_vault_automation_conflicts(target_uid: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if result['has_conflict']:
            ...
            result['message'] = f"Vault automation already active for UID {target_uid} in sessions: {', '.join(active_labels)}"
        else:
            result['message'] = f"No vault automation conflicts for UID {target_uid}"
            
    except Exception as e:
        result['message'] = f'Conflict check error: {str(e)}'
        logging.error(f"[check_vault_automation_conflicts] Error: {e}")
...
def get_uid_vault_summary(target_uid: str) -> Dict[str, Any]:
    ...
    try:
        ...
        logging.error(f"[get_uid_vault_summary] Error: {e}")
