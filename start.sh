...
if [ -f /app/logconfig.yaml ]; then
	exec gosu "$PUID":"$PGID" uvicorn app:app --host 0.0.0.0 --port "${PORT:-39842}" --no-access-log --log-config /app/logconfig.yaml
else
	exec gosu "$PUID":"$PGID" uvicorn app:app --host 0.0.0.0 --port "${PORT:-39842}" --no-access-log --log-level "$uvicorn_loglevel"
fi
