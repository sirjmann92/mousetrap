...
from backend.millionaires_vault_cookies import parse_browser_cookies, get_browser_user_agent
...
def test_browser_detection():
    ...
    for i, case in enumerate(test_cases):
        ...
        # Check results
        assert parsed.get('mam_id') == case['expected_mam_id'], f"mam_id mismatch: {parsed.get('mam_id')} != {case['expected_mam_id']}"
        assert parsed.get('uid') == case['expected_uid'], f"uid mismatch: {parsed.get('uid')} != {case['expected_uid']}"
        assert parsed.get('browser', 'chrome') == case['expected_browser'], f"browser mismatch: {parsed.get('browser', 'chrome')} != {case['expected_browser']}"
        ...
        if browser_type == 'firefox':
            assert 'Firefox' in user_agent, f"Firefox signature not found in user agent"
        elif browser_type == 'chrome':
            assert 'Chrome' in user_agent and 'Edg' not in user_agent, f"Chrome signature not found in user agent"
        elif browser_type == 'edge':
            assert 'Edg' in user_agent, f"Edge signature not found in user agent"
        elif browser_type == 'safari':
            assert 'Safari' in user_agent and 'Chrome' not in user_agent, f"Safari signature not found in user agent"
        elif browser_type == 'opera':
            assert 'OPR' in user_agent, f"Opera signature not found in user agent"
